...
const CATEGORY_KEYWORDS = {
  tiles: ["tile", "tileset", "tilesheet", "tilemap"],
  background: ["background", "layer_far", "layer_mid", "layer_near", "parallax", "bg"],
  decor: ["decor", "prop", "bush", "tree", "rock", "plant", "foliage"],
  player: ["player", "character", "hero", "adventurer", "sprite"],
  fx: ["particle", "smoke", "explosion", "spark", "fx"],
  ui: ["ui", "icon", "hud", "rpg", "button"]
};

const CATEGORY_ORDER = ["player", "fx", "ui", "tiles", "background", "decor"];

const CATEGORY_PATTERNS = Object.fromEntries(
  Object.entries(CATEGORY_KEYWORDS).map(([category, keywords]) => [
    category,
    keywords.map((keyword) => keywordToRegExp(keyword))
  ])
);
...
function keywordToRegExp(keyword) {
  ...
  if (keyword.length <= 2) {
    return new RegExp(`(^|[-_/])${escaped}([-.\\d_]|$)`, "i");
  }
...
function defaultCategoryForSource(sourceName) {
  switch (sourceName) {
    case "platformer-art-pixel-redux":
      return "tiles";
    case "platformer-characters":
      return "player";
    case "particle-pack":
      return "fx";
    case "ui-pack-rpg-expansion":
      return "ui";
    default:
      return null;
  }
...
function pushCandidate(buckets, category, entry) {
...
async function collectProfileFiles(profileSources) {
  ...
  const totals = { tiles: 0, background: 0, decor: 0, player: 0, fx: 0, ui: 0, skipped: 0, total: 0 };
  ...
  for (const source of profileSources) {
    ...
    const packStats = { tiles: 0, background: 0, decor: 0, player: 0, fx: 0, ui: 0, skipped: 0 };
    for (const entry of entries) {
      if (!entry.relative.toLowerCase().endsWith(".png")) {
        continue;
      }
      totals.total += 1;
      let category = categorizeRelativePath(entry.relative);
      if (!category && matchesPattern(entry.relative, source.pick)) {
        category = defaultCategoryForSource(source.name);
      }
      if (!category) {
        packStats.skipped += 1;
        totals.skipped += 1;
      ...
      if (pushCandidate(buckets, category, record)) {
        packStats[category] += 1;
        totals[category] += 1;
      } else {
        packStats.skipped += 1;
        totals.skipped += 1;
    ...
    log(
      `[scan] ${source.name} -> tiles=${packStats.tiles} bg=${packStats.background} decor=${packStats.decor} player=${packStats.player} fx=${packStats.fx} ui=${packStats.ui} skipped=${packStats.skipped}`
    );
  }

  log(
    `[scan] totals -> tiles=${totals.tiles} bg=${totals.background} decor=${totals.decor} player=${totals.player} fx=${totals.fx} ui=${totals.ui} skipped=${totals.skipped} total=${totals.total}`
  );

  return { buckets, totals };
}
function selectKenneyInputs(candidates) {
  const sortByPath = (list) => [...list].sort((a, b) => a.relativePath.localeCompare(b.relativePath));
  const used = new Set();

  const pickByHints = (list, hints = []) => {
    if (!Array.isArray(list) || list.length === 0) {
      return null;
    }
    const normalizedHints = hints.map((hint) => hint.toLowerCase());
    const filtered = list.filter((item) => !used.has(item.fullPath));
    const match = filtered.find((item) =>
      normalizedHints.some((hint) => item.relativePath.toLowerCase().includes(hint))
    );
    const choice = match || filtered[0] || null;
    if (choice) {
      used.add(choice.fullPath);
    }
    return choice;
  };

  const sortedPlayer = sortByPath(candidates.player);
  const sortedTiles = sortByPath(candidates.tiles);
  const sortedBackground = sortByPath(candidates.background);
  const sortedDecor = sortByPath(candidates.decor);
  const sortedFx = sortByPath(candidates.fx);
  const sortedUi = sortByPath(candidates.ui);

  const playerSheet = pickByHints(sortedPlayer, ["player_tilesheet", "player", "spritesheet", "sheet"]);
  const zombieSheet = pickByHints(sortedPlayer, ["zombie"]);
  const backgroundSheet = pickByHints([...sortedBackground, ...sortedDecor], ["background", "bg", "layer"]);

  return {
    playerSheet,
    zombieSheet,
    particleFiles: sortedFx,
    tileFiles: sortedTiles,
    backgroundSheet,
    iconFiles: sortedUi,
    decorFiles: sortedDecor
  };
}
...
async function buildKenneyWorldAtlas(selection) {
  ...
  const tileFrames = [];
  const tileCandidates = selection.tileFiles || [];
  const decorCandidates = selection.decorFiles || [];
  const tilesToUse = tileCandidates.slice(0, 64);
  ...
  let decorFrames = [];
  const extraDecor = [
    ...tileCandidates.slice(64, 96),
    ...decorCandidates.slice(0, 32)
  ];
...
async function detectTileInfo() {
  ...
  try {
    ...
    return { width: TILE_SIZE, height: TILE_SIZE, columns: 8, tilecount: 48 };
  }
}
...
async function main() {
  ...
  const { buckets: candidates, totals } = await collectProfileFiles(sources);
  if (totals.total === 0) {
    log("fallback activated: no candidates");
  }

  const tileInfo = await detectTileInfo();
  log(
    `Tile metadata -> ${tileInfo.width}x${tileInfo.height}, columns=${tileInfo.columns}, count=${tileInfo.tilecount} (using TILE_SIZE=${TILE_SIZE})`
  );
  ...
  try {
    if (totals.total === 0) {
      throw new Error("No PNG candidates found");
    }
    const selection = selectKenneyInputs(candidates);
    ...
    const pngInputs = {
      tiles: candidates.tiles.length,
      background: candidates.background.length,
      decor: candidates.decor.length,
      player: candidates.player.length,
      fx: candidates.fx.length,
      ui: candidates.ui.length
    };
    const totalPngsUsed =
      pngInputs.player + pngInputs.fx + pngInputs.ui + pngInputs.tiles + pngInputs.background + pngInputs.decor;
    log(
      `[kenney] frames -> core=${summary.coreFrames}, world=${summary.worldFrames} (grid ${summary.columns}x${summary.rows}) pngs=${totalPngsUsed}`
    );
  } catch (err) {
  ...
  if (summary) {
    log(
      `[summary] profile=${profileName} coreFrames=${summary.coreFrames} worldFrames=${summary.worldFrames} atlasSize=1024x1024 drawCalls≈${drawCalls} pngSources=${totals.total}`
    );
  } else {
